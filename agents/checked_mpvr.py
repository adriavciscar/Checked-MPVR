...
GEN_EXAMPLES: list[Example] = [
    {
        ...
        "prompts": (
            "\"Describe the visual characteristics of the texture labeled as {category}.\"\n"
            "\"How would you recognize the texture labeled as {category}?\"\n"
            "\"What are the key features of the texture labeled as {category}?\"\n"
            "\"Provide a detailed description of the appearance of the texture labeled as {category}.\"\n"
            "\"If you see an image with the texture labeled as {category}, what would stand out to you?\"\n"
            "\"Imagine you encounter a surface with the texture labeled as {category}. How would you describe it?\"\n"
            "\"What visual attributes define the texture category {category}?\"\n"
            "\"Describe an image featuring the texture labeled as {category}.\"\n"
            "\"Create a caption for an image showcasing the texture labeled as {category}:\"\n"
    ...
    },
    {
        ...
        # For some reason this are raw literals by default so no need to escape.
        "prompts": (
            "\"Describe how does the {category} looks like from a satellite.\"\n"
            "\"How can you recognize the {category} from a satellite?\"\n"
            "\"What does the satellite photo of {category} look like?\"\n"
            "\"Describe the satellite photo from the internet of {category}.\"\n"
            "\"How can you identify the {category} from a satellite?\"\n"
            "\"Explain the geographical features visible in the satellite image of {category}.\"\n"
            "\"Highlight the distinguishing characteristics of {category} in satellite imagery.\"\n"
            "\"What land cover details are discernible in the satellite snapshot of {category}?\"\n"
            "\"Discuss the unique patterns captured in the satellite photo of {category}.\"\n"
            "\"Elaborate on the topographical variations showcased in the satellite view of {category}.\"\n"
        )
    }
]
...
class CheckedMPVRAgent:
    ...
    def get_prompts(self, queries: list[str],
        ...
        for category in tqdm(categories, desc="Categories", position=0):
            ...
            while not completed:
                try:
                    with BatchBarCallback(len(queries),
